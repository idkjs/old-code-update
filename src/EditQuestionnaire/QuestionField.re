...
[@react.component]
let make = (~question: question, ~onChange: question => unit, ~id: string, ()) => {
  let setDescription = setDescription(question) |- onChange;
  let setChoice = (index, choice) =>
    setChoice(question, index, choice) |> onChange;
  ...
  <div className=Style.question>
    {switch (question) {
     ...
     | TextArea(description) =>
       <TextInput label value=description onChange=setDescription />
     | AlternateChoices(description, choices)
     | MultipleChoices(description, choices) =>
       <>
         <TextInput label value=description onChange=setDescription />
